...
void quantize_indices(
    ...
    Buffer<uint64_t> prefix_prods({N, mqd});

    for(uint64_t i = 0; i < N; i++) {
        prefix_prods[i * mqd] = 1;
        quant_dim_nums[i] = 1;
        for(uint64_t j = 1; j < mqd ; j++) {
            if(quantization_dims[i * mqd + j - 1] == 0) {
                break;
            }
            else {
    ...
    #pragma omp parallel
{
    ...
    #pragma omp for
    for(uint64_t i = 0; i < J; i++) {
        std::fill(indices(i * N), indices((i + 1) * N), 0);
        for(uint64_t j = 0; j < NQ; j++) {
            unpermuted[permutation[j]] = quantized_indices[i * NQ + j];
        }

        uint64_t current_position = 0;
        for(uint64_t j = 0; j < N; j++) {
            for(uint64_t k = 0; k < quant_dim_nums[j]; k++) {
                indices[i * N + j] += unpermuted[current_position] * prefix_prods[j * mqd + k];
                current_position++;
            }
        }
    }
